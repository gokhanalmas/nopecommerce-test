...
namespace Nop.Plugin.Payments.TwoCheckout
{
    ...
    public class TwoCheckoutPaymentProcessor : BasePlugin, IPaymentMethod
    {
        ...
        public void PostProcessPayment(PostProcessPaymentRequest postProcessPaymentRequest)
        {
            ...
            //products
            var orderProductVariants = postProcessPaymentRequest.Order.OrderProductVariants.ToList();
            for (int i = 0; i < orderProductVariants.Count; i++)
            {
                int pNum = i + 1;
                var opv = orderProductVariants[i];
                var pv = orderProductVariants[i].ProductVariant;
                ...
                if (!String.IsNullOrEmpty(pv.GetLocalized(x => x.Name)))
                    c_name_value = string.Format("{0} ({1})", pv.Product.GetLocalized(x => x.Name), pv.GetLocalized(x => x.Name));
                else
                    c_name_value = pv.Product.GetLocalized(x => x.Name);
                ...
                string c_description_value = "";
                if (!String.IsNullOrEmpty(pv.GetLocalized(x => x.Name)))
                    c_description_value = string.Format("{0} ({1})", pv.Product.GetLocalized(x => x.Name), pv.GetLocalized(x => x.Name));
                else
                    c_description_value = pv.Product.GetLocalized(x => x.Name);
